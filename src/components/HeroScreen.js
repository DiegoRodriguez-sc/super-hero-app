...
const HeroScreen = ({history}) => {
 ...
 const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`https://www.superheroapi.com/api.php/4205666286179774/${heroId}`)
     .then(response => {
              ...
              <div className="card-body">
                ...
                <div className="mt-2"> 
                  <div className="d-flex justify-content-around">
                   <p className="card-text">Weight: {hero.appearance.weight.map(wei =>  ` ${wei}. `)}</p>
                   <p className="card-text">Height: {hero.appearance.height.map(hei => ` ${hei}. `)}</p>
                  </div>
                </div>
                <div className="mt-2">
                  <div className="d-flex justify-content-around">
